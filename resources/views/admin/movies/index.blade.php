                        ...
                        <tr class="bg-white border-b ">
                            ...
                            <td class="py-4 px-6 text-md font-medium text-right ">
                                <a href="{{route('admin.movie.edit', $movie)}}"
                                   class="text-blue-600  hover:underline">{{__('admin/movie.edit_btn')}}</a>
                            </td>
                            <td class="py-4 px-6 text-md font-medium text-right ">
                                <form action="{{route('admin.movie.destroy',$movie)}}"
                                      method="post"
                                      onsubmit="return confirm('{{__('admin/movie.delete_confirm')}}');">
                                    @csrf
                                    @method('DELETE')
                                    <button class="text-red-600  hover:underline">
                                        {{__('admin/movie.delete')}}
                                    </button>
