            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200 flex flex-col justify-center items-center">
                    @if ($errors->any())
                        ...
                        @endforeach
                    @endif
                    <form class="w-full" action="{{route('admin.movie.update', $movie)}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="flex-mx-3 mb-6 justify-center ">
                            <div class="w-full px-3 mb-6">
                                ...
                                <input
                                    ...
                                    id="grid-first-name" type="text" name="title" value="{{$movie->title}}">
                            </div>
                            <div class="w-full px-3 mb-6">
                                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2"
                                       for="grid-first-name">
                                ...
                                <input
                                    class="appearance-none block w-full bg-gray-50 text-gray border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white"
                                    id="grid-first-name" type="text" name="description" value="{{$movie->description}}">
                            </div>
                            ...
                            <div class="w-full px-3 mb-6">
                                ...
                                <div class="relative">
                                    <select name="category_id" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
                                        <option disabled>{{__('admin/movie.select')}}</option>
                                        @foreach($categories as $category)
                                            <option value="{{$category->id}}" {{$category->id == $movie->category_id ? 'selected' : null}}>{{$category->name}}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="w-full px-3 mb-6">
                                ...
                                <div class="relative">
                                    <select name="chacter_id" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
                                        <option selected disabled>{{__('admin/movie.select')}}</option>
                                        @foreach($artists as $artist)
                                            <option value="{{$artist->id}}" {{$artist->id == $movie->character_id}}>{{$artist->full_name}}</option>
                                        @endforeach
                                    </select>
                                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                                        <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
