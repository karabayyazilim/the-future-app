                    ...
                    @foreach($degrees as $degree)
                        <tr class="bg-white border-b ">
                            ...
                            <td class="py-4 px-6 text-md font-medium text-right ">
                                <a href="{{route('admin.degree.edit', $degree)}}"
                                   class="text-blue-600  hover:underline">{{__('admin/degree.edit_btn')}}</a>
                            </td>
                            <td class="py-4 px-6 text-md font-medium text-right ">
                                <form action="{{route('admin.degree.destroy',$degree)}}"
                                      method="post"
                                      onsubmit="return confirm('{{__('admin/degree.delete_confirm')}}');">
                                    @csrf
                                    @method('DELETE')
                                    <button class="text-red-600  hover:underline">
                                        {{__('admin/degree.delete')}}
                                    </button>
